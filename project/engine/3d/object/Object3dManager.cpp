...
namespace Norm {
	...
	void Object3dManager::SettingCommonDrawing(NameGPS indexNG, StencilRole indexSR) {
		//ルートシグネチャをセットするコマンド
		MainRender::GetInstance()->GetCommandList()->SetGraphicsRootSignature(rootSignature_[(int)indexNG].Get());
		//グラフィックスパイプラインステートをセットするコマンド
		MainRender::GetInstance()->GetCommandList()->SetPipelineState(graphicsPipelineState_[(int)indexNG][(int)indexSR].Get());
	...
	void Object3dManager::SettingPSO() {
		HRESULT hr;

		for (int ngIndex = 0; ngIndex < (int)NameGPS::kMaxNumNameGPS; ngIndex++) {
			for (int srIndex = 0; srIndex < (int)StencilRole::kMaxNumStencilRole; srIndex++) {
				...
				// Transform
				{
					...
					range.BaseShaderRegister = registerCountVS++;
				...
				{
					D3D12_DESCRIPTOR_RANGE range{};
					range.BaseShaderRegister = registerCountPS++;
					range.NumDescriptors = 1;
					range.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
					range.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
					descriptorRanges.push_back(range);
				...
				//バイナリをもとに生成
				hr = DirectXCommon::GetInstance()->GetDevice()->CreateRootSignature(0, signatireBlob->GetBufferPointer(),
					signatireBlob->GetBufferSize(), IID_PPV_ARGS(&rootSignature_[ngIndex]));
				assert(SUCCEEDED(hr));
				...
				if (ngIndex == 0) {
					...
					pixelShaderBlob = DirectXCommon::GetInstance()->CompileShader(L"Resources/shaders/object/Object3d.PS.hlsl",
						L"ps_6_0");
					...
					vertexShaderBlob = DirectXCommon::GetInstance()->CompileShader(L"Resources/shaders/object/Skybox.VS.hlsl",
						L"vs_6_0");
					assert(vertexShaderBlob != nullptr);
					pixelShaderBlob = DirectXCommon::GetInstance()->CompileShader(L"Resources/shaders/object/Skybox.PS.hlsl",
						L"ps_6_0");
					assert(pixelShaderBlob != nullptr);
				}
				...
				switch (srIndex) {
				case (int)Norm::StencilRole::Normal:
					//深度設定
					...
					break;
				case (int)Norm::StencilRole::OlTarget:
					// 深度設定
					depthStencilDesc.DepthEnable = TRUE;
					depthStencilDesc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ALL;
					depthStencilDesc.DepthFunc = D3D12_COMPARISON_FUNC_LESS_EQUAL;
					...
					depthStencilDesc.BackFace = depthStencilDesc.FrontFace;
					break;
				case (int)Norm::StencilRole::Outline:
					// 深度設定
					depthStencilDesc.DepthEnable = TRUE;
				...
				graphicsPipelineStateDesc.pRootSignature = rootSignature_[ngIndex].Get();
				...
				hr = DirectXCommon::GetInstance()->GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc,
					IID_PPV_ARGS(&graphicsPipelineState_[ngIndex][srIndex]));
				assert(SUCCEEDED(hr));
			}
		}
	}
